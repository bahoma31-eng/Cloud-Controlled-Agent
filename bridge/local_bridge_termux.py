...
~~~~~~~~~~~~~~~~~~~~~~
Local Bridge Script V2.3 - Termux Edition
...
- BASH: تنفيذ أوامر Bash (بديل PowerShell الرئيسي)
- SH: تنفيذ أوامر sh
...
# CodeAnalyzer - مسار Termux الافتراضي
CODE_ANALYZER_PATH = os.getenv(
    "CODE_ANALYZER_PATH",
    os.path.expanduser("~/CodeAnalyzer")   # /data/data/com.termux/files/home/CodeAnalyzer
)
...
def execute_command(engine, content, timeout):
    ...
    try:
        ...
        if engine_upper == "PYTHON":
            ...
            if not py:
                return 1, "Engine 'PYTHON' not found. Run: pkg install python"
            cmd = [py, "-c", content]
            rc, stdout, stderr = _run_process(cmd, timeout_value, cwd)

        # ------------------------------------------------------------------ BASH
        elif engine_upper in ("BASH", "POWERSHELL"):
            # POWERSHELL مُعاد توجيهه تلقائيًا إلى bash في Termux
            if engine_upper == "POWERSHELL":
                _log("**", "POWERSHELL -> redirected to BASH on Termux")

            bash = _which_or_none("bash")
            if not bash:
                return 1, "Engine 'BASH' not found. Run: pkg install bash"
            cmd = [bash, "-c", content]
            rc, stdout, stderr = _run_process(cmd, timeout_value, cwd)

        # ------------------------------------------------------------------ SH
        elif engine_upper == "SH":
            sh = _which_or_none("sh")
            if not sh:
                return 1, "Engine 'SH' not found on this system."
            cmd = [sh, "-c", content]
            rc, stdout, stderr = _run_process(cmd, timeout_value, cwd)
        ...
        # ------------------------------------------------------------------ ANALYZER
            ...
            py = _which_or_none("python3") or _which_or_none("python")
            if not py:
                return 1, "Engine 'PYTHON' not found (required for ANALYZER)."

            if not os.path.isdir(CODE_ANALYZER_PATH):
                return 1, (
                    f"CodeAnalyzer directory not found: {CODE_ANALYZER_PATH}
"
                    f"Set CODE_ANALYZER_PATH in .env or copy the folder to ~/CodeAnalyzer"
                )
...
"
                "Available on Termux: PYTHON | BASH | SH | ANALYZER
"
                "Note: POWERSHELL is auto-redirected to BASH."
            )
...
def main():
    ...
    _header("Local Bridge Agent V2.3 - Termux Edition (Auto Execute)")
    ...
    print("  >> Engines: PYTHON | BASH | SH | ANALYZER")
    print("  >> POWERSHELL tasks are auto-redirected to BASH.")
    ...
    while True:
        try:
            ...
            result = {
                ...
                "timestamp":       _now(),
                "executed_on":     "termux_local_machine",
